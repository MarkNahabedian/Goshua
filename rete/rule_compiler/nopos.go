...
func NoPos(n interface{}) interface{} {
	...
	if t := reflect.TypeOf(n); t.Kind() == reflect.Slice {
		nodeType = t.Elem()
	} else {
		if node, ok := n.(ast.Node); !ok {
			panic(fmt.Sprintf("NoPos called on %T, which is not an ast.Node", n))
		} else {
	...
	nopos = func(n interface{}) interface{} {
		...
		case []ast.Decl:
			r := make([]ast.Decl, len(n1))
			for i, d := range n1 {
				r[i] = nopos(d).(ast.Decl)
			}
			return r
		case []ast.Expr:
		...
		default:
			if reflect.TypeOf(n).AssignableTo(nodeType) {
